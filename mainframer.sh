...
CONFIG_USER_FILE="$CONFIG_DIR/user"
...
function readConfigUserProperty {
	grep "^${1}=" "$CONFIG_USER_FILE" | cut -d'=' -f2
}

function printErrorMessage {
    echo
    echo "Error: $1"
    echo "For more information please see to README.md file"
    echo
}

REMOTE_MACHINE_IP_PROPERTY="ip"
REMOTE_MACHINE_USER_PROPERTY="user"
...
if [[ ! -f "${CONFIG_FILE}" ]]; then
	printErrorMessage "Please create and fill ${CONFIG_FILE} file."
	exit 1
fi

if [[ ! -f "${CONFIG_USER_FILE}" ]]; then
	printErrorMessage "Please create and fill ${CONFIG_USER_FILE} file."
	exit 1
fi

USER=$(readConfigUserProperty "$REMOTE_MACHINE_USER_PROPERTY")
IP=$(readConfigProperty "$REMOTE_MACHINE_IP_PROPERTY")
...
if [[ -z "$IP" ]]; then
	printErrorMessage "Please specify \"${REMOTE_MACHINE_IP_PROPERTY}\" property in $CONFIG_FILE."
	exit 1
fi

if [[ -z "$USER" ]]; then
	printErrorMessage "Please specify \"${REMOTE_MACHINE_USER_PROPERTY}\" property in $CONFIG_USER_FILE."
	exit 1
fi
...
function syncBeforeRemoteCommand {
	...
	COMMAND="rsync --progress --archive --delete --rsync-path=\"mkdir -p \"$PROJECT_DIR_ON_REMOTE_MACHINE\" && rsync\" --compress-level=$LOCAL_COMPRESS_LEVEL "
